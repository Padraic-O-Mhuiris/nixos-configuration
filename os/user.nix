...
let
  inherit (lib) mkOption types attrsets flatten mapAttrsToList;

  inherit (types) listOf functionTo attrsOf unspecified;

  inherit (attrsets) foldAttrs recursiveUpdate;
...
in {
  options.__os__.user = mkOption {
    type = listOf (functionTo (attrsOf unspecified));
    description = "";
    default = [ ];
  };
  ...
  config = let
    ...
    userAttrs = lib.flatten (lib.mapAttrsToList
      (user: userConfig: map (fn: fn { inherit user userConfig; }) userClosures)
      users);
